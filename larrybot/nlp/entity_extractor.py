...
class EntityExtractor:
    ...
    def extract_entities(self, text: str) -> dict:
        ...
        entities = {}
        all_dates = []
        # 1. dateparser on whole text
        date = parse_date(text, settings={"PREFER_DATES_FROM": "future"})
        if date:
            all_dates.append(date.isoformat())
        # 2. spaCy NER for DATE/TIME
        for ent in doc.ents:
            if ent.label_ in ["DATE", "TIME"]:
                parsed = parse_date(ent.text, settings={"PREFER_DATES_FROM": "future"})
                if parsed:
                    all_dates.append(parsed.isoformat())
            if ent.label_ in ["PERSON", "ORG", "GPE"]:
                entities.setdefault('named_entities', []).append({"text": ent.text, "label": ent.label_})
        if all_dates:
            entities['date'] = all_dates[0]
            entities['all_dates'] = all_dates
        # 3. Extract task name (simple heuristic: noun chunks after 'task' or 'remind')
