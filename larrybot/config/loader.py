...
class Config:
    ...
    def __init__(self, env_file: str='.env'):
        ...
        self.ALLOWED_TELEGRAM_USER_ID: int = int(os.getenv(
            'ALLOWED_TELEGRAM_USER_ID', '0'))
        self.DATABASE_PATH: str = os.getenv('DATABASE_PATH', 'larrybot.db')
        self.GOOGLE_CLIENT_SECRET_PATH: str = os.getenv(
            'GOOGLE_CLIENT_SECRET_PATH', 'client_secret.json')
        self.LOG_LEVEL: str = os.getenv('LOG_LEVEL', 'INFO')
        self.MAX_REQUESTS_PER_MINUTE: int = int(os.getenv(
            'MAX_REQUESTS_PER_MINUTE', '60'))
        self.NLP_ENABLED: bool = os.getenv('NLP_ENABLED', 'true').lower(
            ) == 'true'
    ...
    def validate(self) ->None:
        ...
        if not self.ALLOWED_TELEGRAM_USER_ID:
            errors.append(
                'ALLOWED_TELEGRAM_USER_ID is required in the environment.')
    ...
    def get_single_user_info(self) ->dict:
        ...
        return {'authorized_user_id': self.ALLOWED_TELEGRAM_USER_ID,
            'bot_token_configured': bool(self.TELEGRAM_BOT_TOKEN),
            'database_path': self.DATABASE_PATH, 'rate_limit_per_minute':
            self.MAX_REQUESTS_PER_MINUTE, 'log_level': self.LOG_LEVEL,
            'nlp_enabled': self.NLP_ENABLED, 'nlp_model': self.NLP_MODEL,
