...
class TaskManager:
    ...
    def _setup_signal_handlers(self):
        ...
        try:
            ...
            logger.debug(
                'Could not setup signal handlers - running in thread context')

    def _signal_handler(self, signum, frame):
        """Handle shutdown signals."""
        if self._shutdown_event.is_set():
            logger.warning(
                f'Received signal {signum} - shutdown already in progress')
            return
        logger.info(
    ...
    def create_task(self, coro, *, name: Optional[str]=None) ->asyncio.Task:
        ...
        logger.debug(
            f"Created task: {task.get_name() if hasattr(task, 'get_name') else 'unnamed'}"
            )
        return task
    ...
    def _task_done_callback(self, task: asyncio.Task):
        ...
        if not task.cancelled():
            try:
                ...
                if exception:
                    logger.error(
                        f"Task {getattr(task, 'get_name', lambda : 'unnamed')()} failed: {exception}"
                        )
    ...
    async def start_background_services(self, test_mode: bool=False):
        ...
        if self._running:
            logger.debug(
                'Background services already running, skipping duplicate start'
                )
            return
        ...
        cache_interval = 2.0 if test_mode else 300.0
        background_interval = 5.0 if test_mode else 1800.0
        try:
            await start_background_processing()
            self.create_task(self._managed_periodic_task(cache_cleanup_task,
                interval=cache_interval, name='cache_cleanup'), name=
                'cache_cleanup')
            logger.info(
                f'✅ Cache cleanup task started (interval: {cache_interval}s)')
            self.create_task(self._managed_periodic_task(
                background_cleanup_task, interval=background_interval, name
                ='background_cleanup'), name='background_cleanup')
            logger.info(
                f'✅ Background maintenance tasks started (interval: {background_interval}s)'
                )
            self._running = True
        except Exception as e:
    ...
    async def _managed_periodic_task(self, task_func, interval: float, name:
        str):
        """
    ...
    async def shutdown(self) ->None:
        ...
        if self._tasks:
            ...
            try:
                    ...
                    self._shutdown_timeout, return_when=asyncio.ALL_COMPLETED)
                for task in done:
                    try:
                        if not task.cancelled():
                            exception = task.exception()
                            if exception:
                                logger.warning(
                                    f"Task {getattr(task, 'get_name', lambda : 'unnamed')()} completed with exception: {exception}"
                                    )
                    except asyncio.CancelledError:
                        pass
                    except Exception as e:
                        logger.debug(
                            f"Error checking task {getattr(task, 'get_name', lambda : 'unnamed')()} status: {e}"
                            )
                if pending:
                    logger.warning(
                        f'⏱️ Shutdown timeout ({self._shutdown_timeout}s), cancelling {len(pending)} remaining tasks...'
                        )
                    for task in pending:
                        task.cancel()
                    try:
                        ...
                        logger.info(
                            '✅ All remaining tasks cancelled successfully')
                    except asyncio.TimeoutError:
                        logger.warning(
    ...
    def get_task_stats(self) ->Dict[str, Any]:
        ...
        return {'total_tasks': len(self._tasks), 'running_tasks': len([t for
            t in self._tasks if not t.done()]), 'completed_tasks': len([t for
            t in self._tasks if t.done() and not t.cancelled()]),
            'cancelled_tasks': len([t for t in self._tasks if t.cancelled()
            ]), 'is_running': self._running, 'shutdown_requested': self.
