...
class LarryBotException(Exception):
    ...
    def __init__(self, message: str, error_code: ErrorCode=ErrorCode.
        UNKNOWN_ERROR, severity: ErrorSeverity=ErrorSeverity.ERROR,
        user_message: Optional[str]=None, context: Optional[Dict[str, Any]]
        =None, suggested_action: Optional[str]=None, original_exception:
        ...
        self.suggested_action = (suggested_action or self.
            _generate_suggested_action())
        self.original_exception = original_exception
        self.timestamp = get_utc_now()
        self.traceback_info = traceback.format_exc(
            ) if original_exception else None
    ...
    def _generate_user_message(self) ->str:
        ...
        user_messages = {ErrorCode.VALIDATION_ERROR:
            ...
            'Plugin error occurred. The system will continue operating.',
            ErrorCode.SERVICE_ERROR:
            'Service temporarily unavailable. Please try again later.'}
        return user_messages.get(self.error_code,
            'An unexpected error occurred. Please try again.')

    def _generate_suggested_action(self) ->str:
        """Generate suggested action based on error code."""
        suggestions = {ErrorCode.VALIDATION_ERROR:
            'Verify your input format and required fields.', ErrorCode.
            DATABASE_ERROR:
            'Wait a moment and try again. Contact support if the issue persists.'
            , ErrorCode.NETWORK_ERROR:
            'Check your internet connection and try again.', ErrorCode.
            AUTH_ERROR: 'Re-authenticate or contact an administrator.',
            ErrorCode.FILE_ERROR: 'Check file permissions and try again.',
            ErrorCode.PLUGIN_ERROR:
            'The system will recover automatically. Report if issues persist.',
            ErrorCode.SERVICE_ERROR:
            'Try again in a few minutes. Check system status if problems continue.'
            }
        return suggestions.get(self.error_code,
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'error_code': self.error_code.value, 'message': self.
            message, 'user_message': self.user_message, 'severity': self.
            severity.value, 'context': self.context, 'suggested_action':
            self.suggested_action, 'timestamp': self.timestamp.isoformat(),
            'original_exception': str(self.original_exception) if self.
            original_exception else None}
...
class ValidationError(LarryBotException):
    ...
    def __init__(self, message: str, field_name: Optional[str]=None,
        ...
        kwargs['error_code'] = kwargs.get('error_code', ErrorCode.
            VALIDATION_ERROR)
...
class NetworkError(LarryBotException):
    ...
    def __init__(self, message: str, url: Optional[str]=None, status_code:
...
class PluginError(LarryBotException):
    ...
    def __init__(self, message: str, plugin_name: Optional[str]=None,
        plugin_operation: Optional[str]=None, **kwargs):
        context = kwargs.get('context', {})
        if plugin_name:
...
def wrap_exception(exception: Exception, error_code: Optional[ErrorCode]=
    None, user_message: Optional[str]=None, context: Optional[Dict[str, Any
    ]]=None) ->LarryBotException:
    """
    ...
    return LarryBotException(message=str(exception), error_code=error_code,
        user_message=user_message, context=context, original_exception=
        exception)


def log_exception(exception: Union[Exception, LarryBotException],
    logger_instance: Optional[logging.Logger]=None, extra_context: Optional
    ...
    if isinstance(exception, LarryBotException):
        ...
        log_level = {ErrorSeverity.INFO: logging.INFO, ErrorSeverity.
            WARNING: logging.WARNING, ErrorSeverity.ERROR: logging.ERROR,
            ErrorSeverity.CRITICAL: logging.CRITICAL}.get(exception.
            severity, logging.ERROR)
        log.log(log_level,
            f'[{exception.error_code.value}] {exception.message}', extra={
            'error_code': exception.error_code.value, 'severity': exception
            .severity.value, 'context': context, 'user_message': exception.
            user_message, 'suggested_action': exception.suggested_action,
...
class ErrorContext:
    ...
    def __init__(self, operation_name: str, logger_instance: Optional[
        logging.Logger]=None, auto_log: bool=True, reraise: bool=True):
        ...
        if exc_type is not None:
            if self.auto_log:
                if isinstance(exc_val, LarryBotException):
                    exc_val.context.update({'operation': self.
                        operation_name, **self.context})
                    log_exception(exc_val, self.logger)
                else:
                    wrapped = wrap_exception(exc_val, context={'operation':
