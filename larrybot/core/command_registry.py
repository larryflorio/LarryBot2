...
class CommandRegistry:
    ...
    def register(self, command: str, handler: Callable[[Any, Any], Any], 
        ...
        # Check if handler has metadata from @command_handler decorator
        if hasattr(handler, '_command_metadata'):
            decorator_metadata = handler._command_metadata
            self._metadata[command] = CommandMetadata(
                name=command,
                description=decorator_metadata.get('description', f"Handler for {command}"),
                usage=decorator_metadata.get('usage', f"{command} [args]"),
                category=decorator_metadata.get('category', 'general')
            )
        elif metadata:
            ...
            self._metadata[command] = CommandMetadata(
                name=command,
                description=f"Handler for {command}",
                usage=f"{command} [args]",
                category="general"
            )
    
    def register_with_middleware(self, command: str, handler: Callable[[Any, Any], Any],
    ...
    def get_commands_by_category(self, category: str) -> List[str]:
        ...
        return [cmd for cmd, meta in self._metadata.items() if meta.category == category]
