...
class TimeZoneService:
    ...
    def _initialize_timezone(self) ->None:
        ...
        try:
            if self._timezone_name:
                ...
                if self._timezone:
                    logger.info(
                        f'Using manually configured timezone: {self._timezone_name}'
                        )
                    return
                else:
                    logger.warning(
                        f"Invalid timezone name '{self._timezone_name}', falling back to automatic detection"
                        )
            self._detected_timezone = self._detect_system_timezone()
            self._timezone = self._detected_timezone
            if self._timezone:
                logger.info(
                    f'Using automatically detected timezone: {self._timezone}')
            else:
        ...
        if ZONEINFO_AVAILABLE and available_timezones:
            for tz_name in available_timezones():
                try:
                    ...
                    tz_offset = tz_now.replace(tzinfo=None) - utc_now.replace(
                        tzinfo=None)
                    if abs((tz_offset - offset).total_seconds()) < 60:
                        return tz
                except Exception:
                    continue
        else:
    ...
    def _get_timezone_by_name(self, timezone_name: str) ->Optional[Union[
        ZoneInfo, Any]]:
        """
    ...
    def reset_to_auto_detection(self) ->bool:
        ...
        if self._detected_timezone:
            ...
            logger.warning(
                'No detected timezone available, keeping current timezone')
            return False
    ...
    def format_datetime(self, dt: datetime, format_str: str='%Y-%m-%d %H:%M:%S'
        ) ->str:
        """
    ...
    def get_timezone_info(self) ->dict:
        ...
        return {'timezone_name': self.timezone_name, 'is_auto_detected': 
            self._detected_timezone == self._timezone, 'detected_timezone':
            str(self._detected_timezone) if self._detected_timezone else
            None, 'manual_override': self._timezone_name,
            'current_local_time': now.isoformat(), 'current_utc_time':
            utc_now.isoformat(), 'utc_offset': now.utcoffset().
            total_seconds() / 3600, 'is_dst': now.dst() != timedelta(0),
...
def initialize_timezone_service(timezone_name: Optional[str]=None
    ) ->TimeZoneService:
    """
