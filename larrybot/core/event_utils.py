...
class EventDataFormatter:
    ...
    def format_task_data(task: Union[Dict[str, Any], Any]) -> Dict[str, Any]:
        ...
        # Handle Task objects (from SQLAlchemy models)
        try:
            return {
                ...
                'actual_hours': float(task.actual_hours) if hasattr(task, 'actual_hours') and task.actual_hours else None,
                'started_at': task.started_at.isoformat() if hasattr(task, 'started_at') and task.started_at else None,
                'parent_id': getattr(task, 'parent_id', None),
                'tags': json.loads(task.tags) if hasattr(task, 'tags') and task.tags else [],
            ...
            # Fallback for any unexpected object types
            return {
                'id': getattr(task, 'id', None),
                'description': str(getattr(task, 'description', 'Unknown Task')),
                'done': getattr(task, 'done', False),
                'priority': getattr(task, 'priority', 'Medium'),
                'due_date': None,
                'category': None,
    ...
    def format_client_data(client: Union[Dict[str, Any], Any]) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'updated_at': client.updated_at.isoformat() if hasattr(client, 'updated_at') else None
            }
        except Exception as e:
            return {
                'id': getattr(client, 'id', None),
                'name': str(getattr(client, 'name', 'Unknown Client')),
                'error': f"Data conversion error: {str(e)}"
            }
...
def safe_event_handler(handler_func):
    ...
    def wrapper(event_data, *args, **kwargs):
        try:
            # Try to call the original handler
            return handler_func(event_data, *args, **kwargs)
        except AttributeError as e:
            # If it's an AttributeError, likely trying to access object attributes on a dict
            ...
            # Re-raise if it's not a format issue
            raise
        except Exception as e:
            # Log the error but don't crash the event system
...
def emit_task_event(event_bus, event_name: str, task_data: Union[Dict[str, Any], Any]) -> None:
    """
...
def emit_client_event(event_bus, event_name: str, client_data: Union[Dict[str, Any], Any]) -> None:
    """
