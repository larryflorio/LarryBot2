...
class ClientRepository:
    ...
    def list_clients(self) ->List[Client]:
        ...
        return self.session.query(Client).options(selectinload(Client.tasks)
    ...
    def get_clients_with_task_counts(self) ->List[dict]:
        ...
        result = self.session.query(Client.id, Client.name, Client.
            created_at, func.count(Task.id).label('task_count'), func.count
            (Task.id).filter(Task.done == False).label('active_task_count')
            ).outerjoin(Task, Client.id == Task.client_id).group_by(Client.
            id, Client.name, Client.created_at).order_by(Client.name.asc()
            ).all()
        return [{'id': row.id, 'name': row.name, 'created_at': row.
            created_at, 'task_count': row.task_count, 'active_task_count':
            row.active_task_count} for row in result]
    ...
    def get_clients_by_ids(self, client_ids: List[int]) ->List[Client]:
        ...
        if not client_ids:
            return []
        return self.session.query(Client).options(selectinload(Client.tasks)
            ).filter(Client.id.in_(client_ids)).order_by(Client.name.asc()
            ).all()

    def search_clients_by_name(self, search_term: str, case_sensitive: bool
        =False) ->List[Client]:
        ...
        if case_sensitive:
            ...
            search_filter = func.lower(Client.name).contains(search_term.
                lower())
        return self.session.query(Client).options(selectinload(Client.tasks)
            ).filter(search_filter).order_by(Client.name.asc()).all()
