...
class CalendarTokenRepository:
    ...
    def add_token(self, provider: str, account_id: str, account_name: str, 
                  ...
                  expiry: Optional[datetime], account_email: Optional[str] = None,
                  is_primary: bool = False) -> CalendarToken:
        """Add a new calendar token with account information."""
        # If this is the first token for the provider, make it primary
        if is_primary or not self.get_all_tokens(provider):
            is_primary = True
        
        # If setting as primary, unset other primary tokens for this provider
        if is_primary:
    ...
    def get_token_by_account(self, provider: str, account_id: str) -> Optional[CalendarToken]:
        """Get token by provider and account ID."""
        return self.session.query(CalendarToken).filter_by(
            provider=provider, 
            account_id=account_id,
            is_active=True
        ).first()
    ...
    def get_all_tokens(self, provider: str) -> List[CalendarToken]:
        ...
        return self.session.query(CalendarToken).filter_by(
            provider=provider,
            is_active=True
        ).order_by(CalendarToken.is_primary.desc(), CalendarToken.account_name).all()
    ...
    def set_primary_account(self, provider: str, account_id: str) -> bool:
        ...
        # Unset all primary tokens for this provider
        self._unset_primary_tokens(provider)
        
        # Set this token as primary
    ...
    def _unset_primary_tokens(self, provider: str) -> None:
        ...
        self.session.query(CalendarToken).filter_by(
            provider=provider,
            is_primary=True
        ).update({"is_primary": False})
        self.session.commit()

    def deactivate_account(self, provider: str, account_id: str) -> Optional[CalendarToken]:
    ...
    def reactivate_account(self, provider: str, account_id: str) -> Optional[CalendarToken]:
        """Reactivate a deactivated account."""
        token = self.session.query(CalendarToken).filter_by(
            provider=provider, 
            account_id=account_id,
            is_active=False
        ).first()
    ...
    def update_token(self, provider: str, account_id: str, 
                    access_token: str, refresh_token: Optional[str], 
                    expiry: Optional[datetime]) -> Optional[CalendarToken]:
    ...
    def rename_account(self, provider: str, account_id: str, new_name: str) -> Optional[CalendarToken]:
        """Rename an account."""
        token = self.get_token_by_account(provider, account_id)
        if not token:
            return None
    ...
    def remove_account(self, provider: str, account_id: str) -> Optional[CalendarToken]:
