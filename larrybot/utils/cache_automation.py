...
class CacheInvalidationRule:
    ...
    conditional_patterns: Optional[Dict[str, List[str]]] = None  # Condition -> patterns
    description: str = ""
...
class AutomatedCacheManager:
    ...
    def _setup_default_rules(self):
        ...
        self.add_rule(CacheInvalidationRule(
            ...
            description="Invalidate task details when updating tasks"
        ))
        ...
        self.add_rule(CacheInvalidationRule(
            ...
            description="Comprehensive cache invalidation for task deletion"
        ))
        
        # Task status change rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.TASK_STATUS_CHANGE,
            cache_patterns=[
                'get_task_by_id',
                'get_tasks_by_status',
                'list_incomplete_tasks',
            ...
            description="Invalidate status-related caches when task status changes"
        ))
        
        # Task priority change rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.TASK_PRIORITY_CHANGE,
            cache_patterns=[
                'get_task_by_id',
                'get_tasks_by_priority'
            ],
            description="Invalidate priority-related caches when task priority changes"
        ))
        
        # Task category change rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.TASK_CATEGORY_CHANGE,
            cache_patterns=[
                'get_task_by_id',
                'get_tasks_by_category',
                'get_all_categories'
            ],
            description="Invalidate category-related caches when task category changes"
        ))
        
        # Task client assignment rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.TASK_CLIENT_CHANGE,
            cache_patterns=[
                'get_task_by_id',
                'get_tasks_by_client'
            ],
            description="Invalidate client-related caches when task assignment changes"
        ))
        
        # Task due date change rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.TASK_DUE_DATE_CHANGE,
            cache_patterns=[
                'get_task_by_id',
                'get_overdue_tasks',
                'get_tasks_due_between'
            ],
            description="Invalidate date-related caches when task due date changes"
        ))
        
        # Bulk operation rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.BULK_OPERATION,
            cache_patterns=[
                'get_tasks_by_status',
                'get_tasks_by_priority',
                'get_tasks_by_category',
            ...
            description="Comprehensive cache invalidation for bulk operations"
        ))
        
        # Client management rules
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.CLIENT_CREATE,
            cache_patterns=['get_all_clients'],
            description="Invalidate client lists when creating clients"
        ))
        
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.CLIENT_UPDATE,
            cache_patterns=['get_all_clients', 'get_client_by_name'],
            description="Invalidate client data when updating clients"
        ))
        
        self.add_rule(CacheInvalidationRule(
            operation_type=OperationType.CLIENT_DELETE,
            cache_patterns=[
                'get_all_clients',
                'get_client_by_name',
                'get_tasks_by_client'
            ],
            description="Invalidate client and related task data when deleting clients"
        ))
    
    def add_rule(self, rule: CacheInvalidationRule):
        """Add a cache invalidation rule."""
        self._rules[rule.operation_type] = rule
        logger.debug(f"Added cache rule for {rule.operation_type.value}: {rule.description}")
    ...
    def invalidate_for_operation(self, operation_type: OperationType, 
        ...
        if not rule:
            logger.warning(f"No cache invalidation rule found for {operation_type.value}")
            return 0
        ...
        # Handle conditional patterns if context provided
            ...
            for condition, patterns in rule.conditional_patterns.items():
                if context.get(condition):
                    for pattern in patterns:
                        ...
                        invalidated_count += count
                        if count > 0:
                            logger.debug(f"Conditionally invalidated {count} cache entries for pattern '{pattern}'")
        
        logger.info(f"Automated cache invalidation for {operation_type.value}: {invalidated_count} total entries invalidated")
        return invalidated_count
    
    def get_rule(self, operation_type: OperationType) -> Optional[CacheInvalidationRule]:
...
def auto_invalidate_cache(operation_type: OperationType, context: Optional[Dict[str, Any]] = None):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                _automated_cache_manager.invalidate_for_operation(operation_type, context)
            except Exception as e:
                logger.error(f"Error in automatic cache invalidation for {operation_type.value}: {e}")
...
def invalidate_caches_for(operation_type: OperationType, context: Optional[Dict[str, Any]] = None) -> int:
    """
    ...
    Returns:
        Number of cache patterns invalidated
    """
    return _automated_cache_manager.invalidate_for_operation(operation_type, context)
