...
def register(event_bus: EventBus, command_registry: CommandRegistry) ->None:
    ...
    command_registry.register('/add', deprecated_add_handler)
    command_registry.register('/tasks', deprecated_tasks_handler)
    command_registry.register('/search_advanced',
        deprecated_search_advanced_handler)
    command_registry.register('/analytics_detailed',
        deprecated_analytics_detailed_handler)
    command_registry.register('/analytics_advanced',
        deprecated_analytics_advanced_handler)
...
async def priority_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    if not context.args[0].isdigit():
        await update.message.reply_text(MessageFormatter.
            format_error_message('Task ID must be a number',
...
async def start_time_tracking_handler(update: Update, context: ContextTypes
    ...
    if not context.args[0].isdigit():
        await update.message.reply_text(MessageFormatter.
            format_error_message('Task ID must be a number',
            'Usage: /time_start <task_id>'), parse_mode='MarkdownV2')
        return
    task_id = int(context.args[0])
    task_service = _get_task_service()
    ...
    if result['success']:
        await update.message.reply_text(MessageFormatter.
            ...
            task_id, 'Status': 'Time tracking started'}), parse_mode=
            'MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
...
async def stop_time_tracking_handler(update: Update, context: ContextTypes.
    ...
    if not context.args[0].isdigit():
        await update.message.reply_text(MessageFormatter.
            format_error_message('Task ID must be a number',
            'Usage: /time_stop <task_id>'), parse_mode='MarkdownV2')
        return
    task_id = int(context.args[0])
    task_service = _get_task_service()
    ...
    if result['success']:
        await update.message.reply_text(MessageFormatter.
            format_success_message(f"⏹️ {result['message']}", {'Task ID':
            task_id, 'Status': 'Time tracking stopped', 'Duration': result.
            get('duration', 'N/A')}), parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Check the task ID and try again.'), parse_mode='MarkdownV2')
...
async def subtask_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    if not context.args[0].isdigit():
        await update.message.reply_text(MessageFormatter.
            format_error_message('Parent task ID must be a number',
    ...
    if result['success']:
        subtask = result['data']
        await update.message.reply_text(MessageFormatter.
            format_success_message(f'✅ Subtask created!', {'Subtask ID':
            subtask['id'], 'Parent Task': parent_id, 'Description': subtask
            ['description']}), parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Check the parent task ID and description.'), parse_mode=
            'MarkdownV2')
...
async def dependency_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    if not context.args[0].isdigit() or not context.args[1].isdigit():
        await update.message.reply_text(MessageFormatter.
            format_error_message('Task IDs must be numbers',
    ...
    if result['success']:
        await update.message.reply_text(MessageFormatter.
            ...
            task_id, 'Depends On': dependency_id}), parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Check both task IDs and ensure they exist.'), parse_mode=
            'MarkdownV2')
...
async def tags_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    if result['success']:
        await update.message.reply_text(MessageFormatter.
            format_success_message(f"✅ {result['message']}", {'Task ID':
            task_id, 'Action': action, 'Tags': ', '.join(tags)}),
            parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
...
async def comment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    if result['success']:
        await update.message.reply_text(MessageFormatter.
            format_success_message(f"✅ {result['message']}", {'Comment':
            comment}), parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Check the task ID and comment text.'), parse_mode='MarkdownV2')
...
async def comments_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    if not context.args[0].isdigit():
        await update.message.reply_text(MessageFormatter.
            format_error_message('Task ID must be a number',
            'Usage: /comments <task_id>'), parse_mode='MarkdownV2')
        return
    task_id = int(context.args[0])
    task_service = _get_task_service()
    ...
    if result['success']:
        ...
        if comments:
            msg = (
                f'💬 **Comments for Task #{task_id}** \\({len(comments)} found\\)\n\n'
                )
            for comment in comments:
                msg += (
                    f"📝 **{MessageFormatter.escape_markdown(comment['comment'])}**\n"
                    )
                msg += f"   📅 {comment['created_at'][:19]}\n\n"
        else:
            msg = f'💬 **Comments for Task #{task_id}**\n\nNo comments found.'
        await update.message.reply_text(escape_markdown_v2(msg), parse_mode
            ='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'], 'Check the task ID.'),
            parse_mode='MarkdownV2')
...
async def advanced_tasks_handler(update: Update, context: ContextTypes.
    ...
    if result['success']:
        ...
        if tasks:
            ...
            if len(tasks) > 0:
                first_task = tasks[0]
                keyboard = KeyboardBuilder.build_task_keyboard(first_task[
                    'id'], first_task['status'])
            ...
            await update.message.reply_text(MessageFormatter.
                ...
                'Try adjusting your filter criteria.'), parse_mode='MarkdownV2'
                )
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
...
async def today_tasks_handler(update: Update, context: ContextTypes.
    ...
    task_service = _get_task_service()
    result = await task_service.get_tasks_with_filters(due_after=
        start_of_day, due_before=end_of_day, done=False)
    if result['success']:
        tasks = result['data']
        if tasks:
...
async def week_tasks_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    task_service = _get_task_service()
    result = await task_service.get_tasks_with_filters(due_after=
        start_of_week_dt, due_before=end_of_week_dt, done=False)
    if result['success']:
        tasks = result['data']
        if tasks:
...
async def search_tasks_handler(update: Update, context: ContextTypes.
    ...
    if not context.args:
        await update.message.reply_text(MessageFormatter.
            format_error_message('Missing search query',
...
• Case-sensitive: /search Project --case-sensitive"""
    ...
    try:
        task_service = _get_task_service()
        if use_advanced:
            result = await task_service.search_tasks_by_text(query,
            ...
            if result['success']:
                ...
                for task in tasks:
                    task_desc = task['description'
                        ] if case_sensitive else task['description'].lower()
                    if search_query in task_desc:
                        matching_tasks.append(task)
                result = {'success': True, 'data': matching_tasks}
        if result['success']:
            tasks = result['data']
            if tasks:
                ...
                title = (
                    f"{search_mode} Search Results for '{query}'{sensitivity}")
                ...
                keyboard_buttons = []
                for task in tasks[:5]:
                    task_row = [UnifiedButtonBuilder.create_button(text=
                        f"👁️ {task['id']}", callback_data=
                        f"task_view:{task['id']}", button_type=ButtonType.
                        INFO), UnifiedButtonBuilder.create_button(text=
                        f"✅ {task['id']}", callback_data=
                        f"task_done:{task['id']}", button_type=ButtonType.
                        INFO), UnifiedButtonBuilder.create_button(text=
                        f"✏️ {task['id']}", callback_data=
                        f"task_edit:{task['id']}", button_type=ButtonType.
                        INFO), UnifiedButtonBuilder.create_button(text=
                        f"🗑️ {task['id']}", callback_data=
                        f"task_delete:{task['id']}", button_type=ButtonType
                        .INFO)]
                ...
                keyboard_buttons.append([UnifiedButtonBuilder.create_button
                    ...
                    'nav_main', button_type=ButtonType.INFO)])
                ...
                search_mode = 'advanced' if use_advanced else 'basic'
                await update.message.reply_text(MessageFormatter.
                    format_info_message('🔍 No Results Found', {'Query':
                    query, 'Mode': search_mode.title(), 'Suggestion':
                    'Try a different search term or use --advanced flag for enhanced search'
                    }), parse_mode='MarkdownV2')
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message(result['message'],
        ...
        if use_advanced:
            ...
            await update.message.reply_text(MessageFormatter.
                format_error_message('Search failed',
                'Please try again with a different query.'), parse_mode=
                'MarkdownV2')


async def _basic_search_fallback(update: Update, query: str, case_sensitive:
    bool=False) ->None:
    ...
    try:
        ...
        if result['success']:
            ...
            matching_tasks = []
            for task in tasks:
                task_desc = task['description'] if case_sensitive else task[
                    'description'].lower()
                if search_query in task_desc:
                    matching_tasks.append(task)
            if matching_tasks:
                message = (
                    f"🔍 Basic search results for '{query}' ({len(matching_tasks)} found):\n\n"
                    )
    ...
    if len(args) > 0:
        ...
        if first_arg in ['basic', 'detailed', 'advanced']:
            ...
            try:
                ...
                await update.message.reply_text(MessageFormatter.
                    format_error_message('Invalid parameter',
    ...
    try:
        ...
        if complexity == 'basic':
            ...
            if result['success']:
                ...
                if analytics.get('daily_completion_trends'):
                    ...
                    trend_data = []
                    for date, count in analytics['daily_completion_trends'
                        ].items():
                        trend_data.append((date, count, f'{count} tasks'))
                    message += ChartBuilder.build_timeline_chart(trend_data,
                        'Daily Completions') + '\n'
                if analytics.get('weekly_patterns'):
                    message += f'📅 **Weekly Patterns**\n'
                    message += ChartBuilder.build_bar_chart(analytics[
                        'weekly_patterns'], 'Tasks by Day of Week') + '\n'
                if analytics.get('priority_trends'):
                    message += f'🎯 **Priority Trends**\n'
                    message += ChartBuilder.build_heatmap(analytics[
                        'priority_trends'], 'Priority Distribution Over Time'
                        ) + '\n'
        ...
        if result['success']:
            keyboard_buttons = [[UnifiedButtonBuilder.create_button(text=
                '📊 Basic', callback_data='analytics_basic', button_type=
                ButtonType.INFO), UnifiedButtonBuilder.create_button(text=
                '📈 Detailed', callback_data='analytics_detailed',
                button_type=ButtonType.INFO), UnifiedButtonBuilder.
                create_button(text='🚀 Advanced', callback_data=
                'analytics_advanced', button_type=ButtonType.INFO)], [
                UnifiedButtonBuilder.create_button(text='📊 Custom Days',
                callback_data='analytics_custom', button_type=ButtonType.
                INFO), UnifiedButtonBuilder.create_button(text=
...
💡 **Tip**: Try `/analytics detailed` or `/analytics advanced` for more insights!"""
        ...
        if complexity != 'basic':
            await update.message.reply_text(MessageFormatter.
                ...
                , 'Generating basic analytics...'), parse_mode='MarkdownV2')
            context.args = []
            await _basic_analytics_fallback(update)
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message('Analytics generation failed',
...
async def _basic_analytics_fallback(update: Update) ->None:
    ...
    try:
        ...
        await update.message.reply_text(
            '❌ Analytics failed. Please try again later.')
...
async def analytics_detailed_handler(update: Update, context: ContextTypes.
    ...
    if result['success']:
        ...
        if analytics.get('performance_metrics'):
            ...
            message += ChartBuilder.build_progress_bar(metrics.get(
                'efficiency', 0), 100, 15, 'Overall Efficiency') + '\n'
            message += ChartBuilder.build_progress_bar(metrics.get(
                'consistency', 0), 100, 15, 'Consistency') + '\n'
        await update.message.reply_text(escape_markdown_v2(message),
            parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
...
Suggested Priority: {suggestion['suggested_priority']}

Use: /addtask {description} {suggestion['suggested_priority']}"""
            )
    else:
        await update.message.reply_text(f"❌ Error: {result['message']}")
...
async def bulk_status_handler(update: Update, context: ContextTypes.
    ...
    try:
        ...
        if status not in valid_statuses:
            await update.message.reply_text(MessageFormatter.
                format_error_message('Invalid status',
                f"Valid statuses: {', '.join(valid_statuses)}"), parse_mode
                ='MarkdownV2')
            return
        task_service = _get_task_service()
        ...
        if result['success']:
            await update.message.reply_text(MessageFormatter.
                format_success_message(f'✅ Bulk Status Update Complete!', {
                'Tasks Updated': len(task_ids), 'New Status': status,
                'Details': result.get('details',
                'All tasks updated successfully')}), parse_mode='MarkdownV2')
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message(result['message'],
                'Please check the task IDs and status value.'), parse_mode=
                'MarkdownV2')
    except ValueError:
        await update.message.reply_text(MessageFormatter.
            format_error_message('Invalid task ID format',
            'Usage: /bulk_status 1,2,3 <status>'), parse_mode='MarkdownV2')
...
async def bulk_priority_handler(update: Update, context: ContextTypes.
    ...
    try:
        ...
        if priority not in valid_priorities:
            await update.message.reply_text(MessageFormatter.
                format_error_message('Invalid priority',
        ...
        if result['success']:
            await update.message.reply_text(MessageFormatter.
                format_success_message(f'✅ Bulk Priority Update Complete!',
        ...
        await update.message.reply_text(MessageFormatter.
            format_error_message('Invalid task ID format',
            'Usage: /bulk_priority 1,2,3 <priority>'), parse_mode='MarkdownV2')
...
async def bulk_assign_handler(update: Update, context: ContextTypes.
    ...
    try:
        ...
        if result['success']:
            await update.message.reply_text(MessageFormatter.
                format_success_message(f'✅ Bulk Assignment Complete!', {
                'Tasks Assigned': len(task_ids), 'Client': client_name,
                'Details': result.get('details',
                'All tasks assigned successfully')}), parse_mode='MarkdownV2')
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message(result['message'],
                'Please check the task IDs and client name.'), parse_mode=
                'MarkdownV2')
    except ValueError:
        await update.message.reply_text(MessageFormatter.
            format_error_message('Invalid task ID format',
            'Usage: /bulk_assign 1,2,3 <client_name>'), parse_mode='MarkdownV2'
            )
...
async def bulk_delete_handler(update: Update, context: ContextTypes.
    ...
    try:
        task_ids = [int(id.strip()) for id in context.args[0].split(',')]
        confirm = len(context.args) > 1 and context.args[1].lower(
            ) == 'confirm'
        if not confirm:
            message = MessageFormatter.format_warning_message(
                f'⚠️ Bulk Delete Confirmation Required', {'Tasks to Delete':
                len(task_ids), 'Task IDs': context.args[0], 'Action':
                'This action cannot be undone', 'Confirmation':
                f'Use: /bulk_delete {context.args[0]} confirm'})
            keyboard = KeyboardBuilder.build_confirmation_keyboard(
                f'bulk_delete_confirm:{context.args[0]}', f'bulk_delete_cancel'
                )
        ...
        if result['success']:
            await update.message.reply_text(MessageFormatter.
                format_success_message(f'🗑️ Bulk Delete Complete!', {
                'Tasks Deleted': len(task_ids), 'Details': result.get(
                'details', 'All tasks deleted successfully'), 'Note':
                'This action cannot be undone'}), parse_mode='MarkdownV2')
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message(result['message'],
        ...
        await update.message.reply_text(MessageFormatter.
            format_error_message('Invalid task ID format',
            'Usage: /bulk_delete 1,2,3 [confirm]'), parse_mode='MarkdownV2')
...
async def bulk_operations_handler(update: Update, context: ContextTypes.
    ...
    message = MessageFormatter.format_info_message('🔄 Bulk Operations Menu',
        {'Status Update': '/bulk_status <ids> <status>', 'Priority Update':
        '/bulk_priority <ids> <priority>', 'Client Assignment':
        '/bulk_assign <ids> <client>', 'Bulk Delete':
        '/bulk_delete <ids> [confirm]', 'Format': 'Task IDs: 1,2,3,4,5'})
...
async def time_entry_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    try:
        ...
        task_service = _get_task_service()
        result = await task_service.add_manual_time_entry(task_id,
        ...
        await update.message.reply_text(
            '❌ Error: Invalid format. Use: /time_entry <task_id> <duration_minutes> [description]'
            )
...
"""
        ...
        await update.message.reply_text(
            '❌ Error: Invalid task ID. Use: /time_summary <task_id>')


@command_handler('/search_advanced', 'Advanced text search',
    'Usage: /search_advanced <search_text>', 'tasks')
@require_args(1, 1)
async def search_advanced_handler(update: Update, context: ContextTypes.
    DEFAULT_TYPE) ->None:
    """Advanced text search in tasks with rich UX."""
    search_text = context.args[0]
    task_service = _get_task_service()
    result = await task_service.search_tasks_by_text(search_text,
    ...
    if result['success']:
        ...
        if tasks:
            ...
            await update.message.reply_text(MessageFormatter.
                ...
                'Try a different search term.'), reply_markup=
                KeyboardBuilder.build_filter_keyboard(), parse_mode=
                'MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Unable to perform search.'), reply_markup=KeyboardBuilder.
            build_filter_keyboard(), parse_mode='MarkdownV2')
...
async def filter_advanced_handler(update: Update, context: ContextTypes.
    ...
    task_service = _get_task_service()
    result = await task_service.get_tasks_with_advanced_filters(status=
        status, priority=priority, category=category, sort_by=sort_by,
        sort_order=sort_order, limit=20)
    if result['success']:
        tasks = result['data']
        if tasks:
            ...
            await update.message.reply_text(MessageFormatter.
                format_error_message(
                'No tasks found with the specified filters.',
                'Try adjusting your filter criteria.'), reply_markup=
                KeyboardBuilder.build_filter_keyboard(), parse_mode=
                'MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Check your filter parameters.'), reply_markup=KeyboardBuilder.
            build_filter_keyboard(), parse_mode='MarkdownV2')
...
async def tags_multi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    try:
        ...
        if result['success']:
            ...
            if tasks:
                title = (
                    f"Tasks matching {match_type} of tags: {', '.join(tags)}")
                message = MessageFormatter.format_task_list(tasks, title)
                keyboard = KeyboardBuilder.build_filter_keyboard()
                await update.message.reply_text(message, reply_markup=
                    keyboard, parse_mode='MarkdownV2')
                ...
                await update.message.reply_text(MessageFormatter.
                    ...
                    , 'Try different tags or match type.'), reply_markup=
                    KeyboardBuilder.build_filter_keyboard(), parse_mode=
                    'MarkdownV2')
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message(result['message'],
                'Check your tag format and try again.'), reply_markup=
                KeyboardBuilder.build_filter_keyboard(), parse_mode=
                'MarkdownV2')
    except Exception as e:
        await update.message.reply_text(MessageFormatter.
            format_error_message('Invalid format for multi-tag filtering.',
            'Usage: /tags_multi <tag1,tag2,tag3> [all|any]'), reply_markup=
            KeyboardBuilder.build_filter_keyboard(), parse_mode='MarkdownV2')
...
async def time_range_handler(update: Update, context: ContextTypes.DEFAULT_TYPE
    ...
    try:
        ...
        include_completed = len(context.args) > 2 and context.args[2].lower(
            ) == 'true'
        task_service = _get_task_service()
        result = await task_service.get_tasks_by_time_range(start_date,
            end_date, include_completed)
        completed_text = ('including completed' if include_completed else
            'excluding completed')
        if result['success']:
            tasks = result['data']
            if tasks:
                title = (
                    f"Tasks in time range {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} ({completed_text})"
                    )
                message = MessageFormatter.format_task_list(tasks, title)
                keyboard = KeyboardBuilder.build_filter_keyboard()
                await update.message.reply_text(message, reply_markup=
                ...
                await update.message.reply_text(MessageFormatter.
                    ...
                    , 'Try a different date range.'), reply_markup=
                    KeyboardBuilder.build_filter_keyboard(), parse_mode=
                    'MarkdownV2')
        else:
            await update.message.reply_text(MessageFormatter.
                format_error_message(result['message'],
                'Unable to filter by time range.'), reply_markup=
                KeyboardBuilder.build_filter_keyboard(), parse_mode=
                'MarkdownV2')
    except ValueError:
        await update.message.reply_text(MessageFormatter.
...
async def priority_range_handler(update: Update, context: ContextTypes.
    ...
    task_service = _get_task_service()
    result = await task_service.get_tasks_by_priority_range(min_priority,
        max_priority)
    if result['success']:
        tasks = result['data']
        if tasks:
            ...
            await update.message.reply_text(MessageFormatter.
                ...
                , 'Try a different range.'), reply_markup=KeyboardBuilder.
                build_filter_keyboard(), parse_mode='MarkdownV2')
    else:
        await update.message.reply_text(MessageFormatter.
            format_error_message(result['message'],
            'Unable to filter by priority range.'), reply_markup=
            KeyboardBuilder.build_filter_keyboard(), parse_mode='MarkdownV2')


@command_handler('/analytics_advanced', 'Advanced analytics',
    'Usage: /analytics_advanced [days]', 'tasks')
async def analytics_advanced_handler(update: Update, context: ContextTypes.
    DEFAULT_TYPE) ->None:
    ...
    try:
        ...
        await update.message.reply_text(MessageFormatter.
            format_error_message('Invalid days format for analytics.',
            'Usage: /analytics_advanced [days]'), reply_markup=
            KeyboardBuilder.build_analytics_keyboard(), parse_mode='MarkdownV2'
            )
...
async def productivity_report_handler(update: Update, context: ContextTypes
    ...
    try:
        ...
        end_date = datetime.strptime(context.args[1], '%Y-%m-%d')
        task_service = _get_task_service()
        result = await task_service.get_productivity_report(start_date,
            end_date)
        if result['success']:
            report = result['data']
...
async def deprecated_tasks_handler(update: Update, context: ContextTypes.
    ...
    await update.message.reply_text(MessageFormatter.format_info_message(
        ...
        'Unified task listing with optional filtering'}), parse_mode=
        'MarkdownV2')
...
async def deprecated_search_advanced_handler(update: Update, context:
    ...
    await update.message.reply_text(MessageFormatter.format_info_message(
        ...
        'New Command': '/search', 'Migration':
        'The /search_advanced command has been merged into /search',
        'Usage': '/search <query> --advanced', 'Benefits':
        'Unified search with basic and advanced modes'}), parse_mode=
        'MarkdownV2')
    if context.args:
        ...
        await update.message.reply_text(MessageFormatter.
            format_error_message('Missing search query',
            'Usage: /search <query> --advanced'), parse_mode='MarkdownV2')
...
async def deprecated_analytics_detailed_handler(update: Update, context:
    ...
    await update.message.reply_text(MessageFormatter.format_info_message(
        ...
        'New Command': '/analytics', 'Migration':
        'The /analytics_detailed command has been merged into /analytics',
        'Usage': '/analytics detailed [days]', 'Benefits':
        'Unified analytics with multiple complexity levels'}), parse_mode=
        'MarkdownV2')
    new_args = ['detailed']
    if context.args:
...
async def deprecated_analytics_advanced_handler(update: Update, context:
    ...
    await update.message.reply_text(MessageFormatter.format_info_message(
        ...
        'New Command': '/analytics', 'Migration':
        'The /analytics_advanced command has been merged into /analytics',
...
async def deprecated_start_handler(update: Update, context: ContextTypes.
    ...
    await update.message.reply_text(MessageFormatter.format_info_message(
        ...
        '/time_start', 'Reason':
        "Namespace conflict resolution with bot's core /start command",
        'Usage': '/time_start <task_id>', 'Benefits':
        'Clearer command naming and no conflicts'}), parse_mode='MarkdownV2')
...
async def deprecated_stop_handler(update: Update, context: ContextTypes.
    ...
    await update.message.reply_text(MessageFormatter.format_info_message(
        ...
        '/time_stop', 'Reason': 'Consistency with /time_start naming',
        'Usage': '/time_stop <task_id>', 'Benefits':
        'Consistent time tracking command naming'}), parse_mode='MarkdownV2')
