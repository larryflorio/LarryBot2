        ...
        try:
            if operation == "process_data":
                return self._create_success_response(
                    {"processed": data.get("input", "").upper()},
...
@command_handler(
    command="/example",
    description="Example command demonstrating new features",
...
async def calculate_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        result = await example_service.execute("calculate", {"numbers": numbers})
        
        if result["success"]:
            await update.message.reply_text(
                f"📊 Calculation Result:\n"
                f"Sum: {result['data']['sum']}\n"
                f"Numbers processed: {result['data']['count']}"
            )
        else:
            await update.message.reply_text(f"❌ Error: {result['error']}")
    except ValueError:
...
@command_handler(
    ...
    description="Show help for example commands",
    usage="/help_examples",
    category="examples"
)
async def help_examples_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
def register(event_bus: EventBus, command_registry: CommandRegistry) -> None:
    ...
    command_registry.register("/calculate", calculate_handler, CommandMetadata(
        name="/calculate", 
        description="Calculate sum of numbers",
        usage="/calculate <num1> <num2> [num3...]",
        category="examples"
    ...
    command_registry.register("/help_examples", help_examples_handler, CommandMetadata(
        name="/help_examples",
        description="Show help for example commands", 
        usage="/help_examples",
        category="examples"
...
# Plugin metadata for dependency injection
PLUGIN_METADATA = {
    "name": "example_enhanced",
    "version": "1.0.0",
    "description": "Example plugin demonstrating enhanced architectural features",
    "author": "LarryBot Team",
    "dependencies": ["event_bus", "command_registry"],
    "enabled": True
} 