...
# Global reference to event bus for attachment events
...
def register(event_bus: EventBus, command_registry: CommandRegistry) -> None:
    ...
    command_registry.register("/attachment_description", update_description_handler)
...
@command_handler("/attach", "Attach file to task", "Usage: /attach <task_id> [description]", "attachments")
@require_args(1, 2)
async def attach_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle file attachment via command with file upload."""
    try:
        # Check if message has document or photo
            ...
            await update.message.reply_text(
                MessageFormatter.format_info_message(
                    "📎 File Attachment Required",
                    {
                        ...
                        "Usage": "Send a file, then reply with: /attach <task_id> [description]",
        ...
        if result['success']:
            ...
            # Create success message with rich formatting
            message = MessageFormatter.format_success_message(
                f"📎 File attached successfully!",
                {
                    "File": attachment_data['filename'],
                    "Task": f"#{task_id}",
                    "Size": f"{attachment_data['size']:,} bytes",
                    "Type": attachment_data['mime_type'],
            ...
            await update.message.reply_text(
                MessageFormatter.format_error_message(
                    result['error'],
                    "Please check the task ID and file format."
                ),
                parse_mode='MarkdownV2'
            )
...
@command_handler("/attachments", "List task attachments", "Usage: /attachments <task_id>", "attachments")
@require_args(1, 1)
async def list_attachments_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if result['success']:
            ...
            if not attachments:
                await update.message.reply_text(
                    MessageFormatter.format_info_message(
                        ...
                        {"Task": f"#{task_id}", "Status": "No files attached"}
                    ),
                    parse_mode='MarkdownV2'
                )
                return
            ...
            message += f"📋 **Files**\n"
            for i, att in enumerate(attachments[:5], 1):  # Show first 5
                message += f"{i}\\. **{MessageFormatter.escape_markdown(att['filename'])}**\n"
                message += f"   📏 {att['size']:,} bytes \\| 📅 {att['created_at'][:19]}\n"
                if att['description']:
                    message += f"   📝 {MessageFormatter.escape_markdown(att['description'])}\n"
                message += "\n"
            
            if len(attachments) > 5:
                message += f"*... and {len(attachments) - 5} more files*\n\n"
            
            # Create keyboard for attachment management
            keyboard = KeyboardBuilder.build_attachments_list_keyboard(task_id, len(attachments))
...
@command_handler("/remove_attachment", "Remove attachment", "Usage: /remove_attachment <attachment_id>", "attachments")
@require_args(1, 1)
async def remove_attachment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if result['success']:
            await update.message.reply_text(
                MessageFormatter.format_success_message(
                    f"🗑️ Attachment removed successfully!",
                    {
                        "File": result['data']['filename'],
                        "ID": str(attachment_id),
                        "Size": f"{result['data']['size']:,} bytes"
                    }
                ),
                parse_mode='MarkdownV2'
            )
            ...
            await update.message.reply_text(
                MessageFormatter.format_error_message(
                    result['error'],
                    "Please check the attachment ID."
                ),
                parse_mode='MarkdownV2'
            )
...
@command_handler("/attachment_description", "Update attachment description", "Usage: /attachment_description <attachment_id> <description>", "attachments")
@require_args(2, 2)
async def update_description_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        attachment_service = _get_attachment_service()
        result = await attachment_service.update_attachment_description(attachment_id, description)
...
@command_handler("/attachment_stats", "Show attachment statistics", "Usage: /attachment_stats [task_id]", "attachments")
async def attachment_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if result['success']:
            ...
            message += f"• Total Size: {stats['total_size']:,} bytes \\({stats['total_size_mb']:.2f} MB\\)\n"
            message += f"• Average Size: {stats['avg_size']:,} bytes\n"
            message += f"• Largest File: {stats['largest_file_size']:,} bytes\n\n"
            ...
            if stats.get('type_distribution'):
                ...
                for file_type, count in stats['type_distribution'].items():
                    ...
                    message += f"• {file_type}: {count} \\({percentage:.1f}%\\)\n"
                message += "\n"
