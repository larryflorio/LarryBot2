...
async def health_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        status_emoji = ('🟢' if overall_status == 'healthy' else '🟡' if 
            overall_status == 'warning' else '🔴')
        message += (
            f'{status_emoji} **Overall Status**: {overall_status.upper()}\n\n')
        db = health_status['database']
        db_emoji = '🟢' if db['status'] == 'healthy' else '🟡' if db['status'
            ] == 'warning' else '🔴'
        ...
        mem_emoji = '🟢' if memory['status'] == 'healthy' else '🟡' if memory[
            'status'] == 'warning' else '🔴'
        ...
        cpu_emoji = '🟢' if cpu['status'] == 'healthy' else '🟡' if cpu['status'
            ] == 'warning' else '🔴'
        ...
        disk_emoji = '🟢' if disk['status'] == 'healthy' else '🟡' if disk[
            'status'] == 'warning' else '🔴'
        ...
        plugins_emoji = '🟢' if plugins['status'
            ] == 'healthy' else '🟡' if plugins['status'] == 'warning' else '🔴'
        message += (
            f"{plugins_emoji} **Plugins**: {plugins['status'].upper()}\n")
        message += (
            f"   • Loaded: {plugins['enabled_plugins']}/{plugins['loaded_plugins']}\n"
            )
        if plugins.get('note'):
...
@command_handler('/health_quick', 'Quick health status',
    'Usage: /health_quick', 'system')
...
@command_handler('/health_detailed', 'Detailed health status',
    'Usage: /health_detailed', 'system')
async def detailed_health_check(update: Update, context: ContextTypes.
    DEFAULT_TYPE):
    ...
    try:
        ...
        if 'system_info' in health_status:
            ...
            message += (
                f"   • Uptime: {sys_info['uptime_seconds'] / 3600:.1f} hours\n\n"
                )
        for component, status in health_status.items():
            if component in ['overall_status', 'timestamp', 'system_info']:
                continue
            emoji = '🟢' if status['status'] == 'healthy' else '🟡' if status[
                'status'] == 'warning' else '🔴'
            message += (
                f"{emoji} **{component.title()}**: {status['status'].upper()}\n"
                )
            if component == 'database':
                message += f"   • Connection: {status['connection']}\n"
                message += (
                    f"   • Size: {status.get('database_size_mb', 'N/A')} MB\n")
                ...
                message += (
                    f"   • Active: {status['enabled_plugins']}/{status['loaded_plugins']}\n"
                    )
            message += '\n'
        await update.message.reply_text(message, parse_mode='Markdown')
    except Exception as e:
...
PLUGIN_METADATA = {'name': 'health', 'version': '1.0.0', 'description':
    'System health monitoring and status reporting', 'author':
    'LarryBot2 Team', 'dependencies': ['health_service'], 'enabled': True}
