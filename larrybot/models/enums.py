...
class TaskStatus(Enum):
    ...
    def can_transition_to(self) -> List['TaskStatus']:
        ...
        transitions = {
            ...
            TaskStatus.IN_PROGRESS: [TaskStatus.REVIEW, TaskStatus.DONE, TaskStatus.BLOCKED, TaskStatus.TODO],
            TaskStatus.REVIEW: [TaskStatus.DONE, TaskStatus.IN_PROGRESS, TaskStatus.TODO],
    ...
    def get_display_options(cls) -> Dict[str, str]:
        ...
        return {status.value: f"{status.color_code} {status.value}" for status in cls}
...
class TaskPriority(IntEnum):
    ...
    def description(self) -> str:
        ...
        descriptions = {
            TaskPriority.LOW: "Low priority - can be done when time permits",
            TaskPriority.MEDIUM: "Medium priority - normal workflow item",
            TaskPriority.HIGH: "High priority - should be completed soon",
            TaskPriority.CRITICAL: "Critical priority - needs immediate attention",
    ...
    def sla_hours(self) -> Optional[int]:
        ...
        sla_mapping = {
            TaskPriority.LOW: None,  # No SLA
            TaskPriority.MEDIUM: 72,  # 3 days
            TaskPriority.HIGH: 24,    # 1 day
            TaskPriority.CRITICAL: 4, # 4 hours
            TaskPriority.URGENT: 1    # 1 hour
        }
    ...
    def get_display_options(cls) -> Dict[str, str]:
        ...
        return {
            priority.name: f"{priority.color_code} {priority.name.title()}"
            for priority in cls
        }
...
class ReminderType(Enum):
    ...
    def description(self) -> str:
        ...
        descriptions = {
            ...
            ReminderType.DAILY: "Repeats every day",
            ReminderType.WEEKLY: "Repeats every week",
            ReminderType.MONTHLY: "Repeats every month",
            ReminderType.YEARLY: "Repeats every year",
            ReminderType.CUSTOM: "Custom repeat pattern"
        }
        return descriptions[self]
...
class FileAttachmentType(Enum):
    ...
    def allowed_extensions(self) -> List[str]:
        ...
        extensions = {
            FileAttachmentType.DOCUMENT: ['.pdf', '.doc', '.docx', '.txt', '.md', '.rtf'],
            FileAttachmentType.IMAGE: ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.svg'],
            FileAttachmentType.AUDIO: ['.mp3', '.wav', '.ogg', '.m4a', '.aac'],
            FileAttachmentType.VIDEO: ['.mp4', '.avi', '.mov', '.wmv', '.mkv'],
            FileAttachmentType.ARCHIVE: ['.zip', '.rar', '.7z', '.tar', '.gz'],
            FileAttachmentType.OTHER: []  # No restrictions
        }
    ...
    def max_size_mb(self) -> Optional[int]:
        ...
        sizes = {
            FileAttachmentType.DOCUMENT: 50,
            FileAttachmentType.IMAGE: 25,
            FileAttachmentType.AUDIO: 100,
            FileAttachmentType.VIDEO: 500,
            FileAttachmentType.ARCHIVE: 200,
            FileAttachmentType.OTHER: 100
        }
        return sizes[self]
...
class AnalyticsTimeRange(Enum):
    ...
    def description(self) -> str:
        """Get human-readable description."""
        descriptions = {
            AnalyticsTimeRange.TODAY: "Today's activity",
            AnalyticsTimeRange.YESTERDAY: "Yesterday's activity",
            AnalyticsTimeRange.THIS_WEEK: "This week (Monday to Sunday)",
            AnalyticsTimeRange.LAST_WEEK: "Last week",
            AnalyticsTimeRange.THIS_MONTH: "This month",
            AnalyticsTimeRange.LAST_MONTH: "Last month",
            AnalyticsTimeRange.THIS_QUARTER: "This quarter",
            AnalyticsTimeRange.THIS_YEAR: "This year",
            AnalyticsTimeRange.LAST_30_DAYS: "Last 30 days",
            AnalyticsTimeRange.LAST_90_DAYS: "Last 90 days",
            AnalyticsTimeRange.ALL_TIME: "All recorded data"
        }
        return descriptions[self]


# Type validation helpers
def validate_enum_value(enum_class: type, value: Any, field_name: str = "field") -> Any:
...
def enum_to_dict(enum_class: type) -> Dict[str, Any]:
    ...
    return {
        'choices': [{'value': item.value, 'label': str(item)} for item in enum_class],
        'default': list(enum_class)[0].value if enum_class else None
    } 