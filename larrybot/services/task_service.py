...
class TaskService(BaseService):
    ...
    async def create_task_with_metadata(self, description: str, priority:
        str='Medium', due_date: Optional[datetime]=None, category: Optional
        [str]=None, estimated_hours: Optional[float]=None, tags: Optional[
        List[str]]=None, parent_id: Optional[int]=None, client_id: Optional
        ...
        try:
            ...
            task = self.task_repository.add_task_with_metadata(description=
                description, priority=priority, due_date=due_date, category
                =category, estimated_hours=estimated_hours, tags=tags,
    ...
    async def get_tasks_with_filters(self, status: Optional[str]=None,
        ...
        parent_id: Optional[int]=None, done: Optional[bool]=None) ->Dict[
        str, Any]:
        """Get tasks with advanced filtering."""
        try:
            tasks = self.task_repository.get_tasks_with_filters(status=
                status, priority=priority, category=category, due_before=
                due_before, due_after=due_after, overdue_only=overdue_only,
                client_id=client_id, parent_id=parent_id, done=done)
            return self._create_success_response([self._task_to_dict(task) for
                task in tasks], f'Found {len(tasks)} tasks')
        except Exception as e:
            return self._handle_error(e, 'Error retrieving tasks')

    async def update_task_priority(self, task_id: int, priority: str) ->Dict[
        str, Any]:
    ...
    async def update_task_due_date(self, task_id: int, due_date: datetime
        ) ->Dict[str, Any]:
    ...
    async def update_task_category(self, task_id: int, category: str) ->Dict[
        str, Any]:
    ...
    async def start_time_tracking(self, task_id: int) ->Dict[str, Any]:
        ...
        try:
            ...
            if task.started_at:
                return self._handle_error(ValueError(
                    f'Time tracking already started for task {task_id}'))
            success = self.task_repository.start_time_tracking(task_id)
            if not success:
                return self._handle_error(ValueError(
                    f'Failed to start time tracking for task {task_id}'))
    ...
    async def stop_time_tracking(self, task_id: int) ->Dict[str, Any]:
        ...
        try:
            ...
            if not task.started_at:
                return self._handle_error(ValueError(
                    f'Time tracking not started for task {task_id}'))
            duration = self.task_repository.stop_time_tracking(task_id)
            if duration is None:
                return self._handle_error(ValueError(
                    f'Failed to stop time tracking for task {task_id}'))
            return self._create_success_response({'task_id': task_id,
    ...
    async def add_subtask(self, parent_id: int, description: str) ->Dict[
        str, Any]:
        ...
        try:
            ...
            if not subtask:
                return self._handle_error(ValueError(
                    f'Failed to create subtask'))
            return self._create_success_response(self._task_to_dict(subtask
                ),
    ...
    async def add_task_dependency(self, task_id: int, dependency_id: int
        ) ->Dict[str, Any]:
        ...
        try:
            ...
            if not dependency:
                return self._handle_error(ValueError(
                    f'Dependency task {dependency_id} not found'))
            if task_id == dependency_id:
                return self._handle_error(ValueError(
                    'Task cannot depend on itself'))
            success = self.task_repository.add_task_dependency(task_id,
                dependency_id)
            if not success:
                return self._handle_error(ValueError(
                    f'Dependency already exists or failed to create'))
            return self._create_success_response({'task_id': task_id,
    ...
    async def get_task_dependencies(self, task_id: int) ->Dict[str, Any]:
        ...
        try:
            ...
            return self._create_success_response([self._task_to_dict(dep) for
                dep in dependencies],
    ...
    async def add_tags(self, task_id: int, tags: List[str]) ->Dict[str, Any]:
        ...
        try:
            ...
            if not updated_task:
                return self._handle_error(ValueError(
                    f'Failed to add tags to task {task_id}'))
            return self._create_success_response(self._task_to_dict(
                updated_task), f'Tags {tags} added to task {task_id}')
        except Exception as e:
    ...
    async def get_tasks_by_tag(self, tag: str) ->Dict[str, Any]:
        ...
        try:
            ...
            return self._create_success_response([self._task_to_dict(task) for
                task in tasks], f"Found {len(tasks)} tasks with tag '{tag}'")
        except Exception as e:
    ...
    async def add_comment(self, task_id: int, comment: str) ->Dict[str, Any]:
        ...
        try:
            ...
            if not task_comment:
                return self._handle_error(ValueError(
                    f'Failed to add comment to task {task_id}'))
            return self._create_success_response(self._comment_to_dict(
                task_comment), f'Comment added to task {task_id}')
        except Exception as e:
    ...
    async def get_task_analytics(self) ->Dict[str, Any]:
        ...
        try:
            ...
            analytics = {**stats, 'overdue_tasks_details': [self.
                _task_to_dict(task) for task in overdue_tasks],
                'tasks_due_today': [self._task_to_dict(task) for task in
                tasks_due_today], 'categories': self.task_repository.
                get_all_categories()}
    ...
    async def suggest_priority(self, description: str) ->Dict[str, Any]:
        ...
        try:
            ...
            high_priority_keywords = ['urgent', 'critical', 'deadline',
                'emergency', 'asap', 'important']
            if any(keyword in description_lower for keyword in
                high_priority_keywords):
                suggested_priority = 'High'
            elif any(word in description_lower for word in ['meeting',
                'call', 'review', 'presentation']):
                suggested_priority = 'Medium'
            else:
                suggested_priority = 'Low'
            return self._create_success_response({'suggested_priority':
                suggested_priority, 'reasoning':
                f'Based on keywords in description: {description}'},
                f'Suggested priority: {suggested_priority}')
        except Exception as e:
            return self._handle_error(e, 'Error suggesting priority')

    async def bulk_update_status(self, task_ids: List[int], status: str
        ) ->Dict[str, Any]:
        ...
        try:
            ...
            for task_id in task_ids:
                ...
                if not task:
                    return self._handle_error(ValueError(
                        f'Task {task_id} not found'))
            updated_count = self.task_repository.bulk_update_status(task_ids,
                status)
            return self._create_success_response({'updated_count':
                updated_count, 'task_ids': task_ids, 'new_status': status},
                f"Updated status to '{status}' for {updated_count} tasks")
        except Exception as e:
            return self._handle_error(e, 'Error updating task statuses')

    async def bulk_update_priority(self, task_ids: List[int], priority: str
        ) ->Dict[str, Any]:
        ...
        try:
            ...
            for task_id in task_ids:
                ...
                if not task:
                    return self._handle_error(ValueError(
                        f'Task {task_id} not found'))
            updated_count = self.task_repository.bulk_update_priority(task_ids,
                priority)
            return self._create_success_response({'updated_count':
                updated_count, 'task_ids': task_ids, 'new_priority':
                priority},
                f"Updated priority to '{priority}' for {updated_count} tasks")
        except Exception as e:
            return self._handle_error(e, 'Error updating task priorities')

    async def bulk_assign_to_client(self, task_ids: List[int], client_name: str
        ) ->Dict[str, Any]:
        ...
        try:
            for task_id in task_ids:
                ...
                if not task:
                    return self._handle_error(ValueError(
                        f'Task {task_id} not found'))
            updated_count = self.task_repository.bulk_assign_to_client(task_ids
                , client_name)
            if updated_count == 0:
                return self._handle_error(ValueError(
                    f"Client '{client_name}' not found"))
            return self._create_success_response({'updated_count':
                updated_count, 'task_ids': task_ids, 'client_name':
                client_name},
    ...
    async def bulk_delete_tasks(self, task_ids: List[int]) ->Dict[str, Any]:
        ...
        try:
            ...
            return self._create_success_response({'deleted_count':
                deleted_count, 'task_ids': task_ids},
                f'Deleted {deleted_count} tasks')
        except Exception as e:
            return self._handle_error(e, 'Error deleting tasks')

    async def add_manual_time_entry(self, task_id: int, duration_minutes:
        int, description: str='') ->Dict[str, Any]:
        ...
        try:
            ...
            success = self.task_repository.add_time_entry(task_id,
                start_time, end_time, description)
            if not success:
                return self._handle_error(ValueError(
                    f'Failed to add time entry for task {task_id}'))
            return self._create_success_response({'task_id': task_id,
                'duration_minutes': duration_minutes, 'description':
                description, 'start_time': start_time, 'end_time': end_time
    ...
    async def search_tasks_by_text(self, search_text: str, case_sensitive:
        ...
        try:
            ...
            tasks = self.task_repository.search_tasks_by_text(search_text,
                case_sensitive)
            return self._create_success_response([self._task_to_dict(task) for
                task in tasks],
    ...
    async def get_tasks_with_advanced_filters(self, status: Optional[str]=
        None, priority: Optional[str]=None, category: Optional[str]=None,
        due_before: Optional[datetime]=None, due_after: Optional[datetime]=
        None, overdue_only: bool=False, client_id: Optional[int]=None,
        ...
        try:
            ...
            tasks = self.task_repository.get_tasks_with_advanced_filters(status
                =status, priority=priority, category=category, due_before=
                due_before, due_after=due_after, overdue_only=overdue_only,
                client_id=client_id, parent_id=parent_id, done=done, tags=
                tags, has_comments=has_comments, has_time_entries=
                has_time_entries, estimated_hours_min=estimated_hours_min,
                estimated_hours_max=estimated_hours_max, created_after=
                created_after, created_before=created_before, sort_by=
                sort_by, sort_order=sort_order, limit=limit)
            return self._create_success_response([self._task_to_dict(task) for
                task in tasks],
                f'Found {len(tasks)} tasks with advanced filters')
        except Exception as e:
            return self._handle_error(e,
                'Error retrieving tasks with advanced filters')

    async def get_tasks_by_multiple_tags(self, tags: List[str], match_all:
        bool=False) ->Dict[str, Any]:
        """Get tasks by multiple tags with option to match all or any."""
        try:
            if not tags:
                return self._handle_error(ValueError('No tags provided'))
            tasks = self.task_repository.get_tasks_by_multiple_tags(tags,
                match_all)
    ...
    async def get_tasks_by_time_range(self, start_date: datetime, end_date:
        datetime, include_completed: bool=True) ->Dict[str, Any]:
        """Get tasks created or due within a specific time range."""
        try:
            if start_date >= end_date:
                return self._handle_error(ValueError(
                    'Start date must be before end date'))
            tasks = self.task_repository.get_tasks_by_time_range(start_date,
    ...
    async def get_tasks_by_priority_range(self, min_priority: str,
        max_priority: str) ->Dict[str, Any]:
        ...
        try:
            ...
            tasks = self.task_repository.get_tasks_by_priority_range(
            ...
            return self._handle_error(e,
                'Error retrieving tasks by priority range')
    ...
    async def get_productivity_report(self, start_date: datetime, end_date:
        ...
        try:
            ...
            report = self.task_repository.get_productivity_report(start_date,
                end_date)
    ...
    def _task_to_dict(self, task: Task) ->Dict[str, Any]:
        ...
        return {'id': task.id, 'description': task.description, 'done':
            task.done, 'priority': task.priority_enum.name.title(),
            'due_date': task.due_date.isoformat() if task.due_date else
            None, 'category': task.category, 'status': task.status,
            'estimated_hours': float(task.estimated_hours) if task.
            estimated_hours else None, 'actual_hours': float(task.
            actual_hours) if task.actual_hours else None, 'started_at': 
            task.started_at.isoformat() if task.started_at else None,
            'parent_id': task.parent_id, 'tags': json.loads(task.tags) if
            task.tags else [], 'client_id': task.client_id, 'created_at':
            task.created_at.isoformat(), 'updated_at': task.updated_at.
            isoformat()}

    def _comment_to_dict(self, comment: TaskComment) ->Dict[str, Any]:
        """Convert comment model to dictionary."""
        return {'id': comment.id, 'task_id': comment.task_id, 'comment':
            comment.comment, 'created_at': comment.created_at.isoformat()}

    def _handle_error(self, error: Exception, context: str='') ->Dict[str, Any
        ]:
        """Handle errors consistently across services."""
        return {'success': False, 'message': f'{context}: {str(error)}' if
            context else str(error)}
