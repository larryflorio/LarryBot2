    ...
    async def _check_plugins_health(self) -> Dict[str, Any]:
        ...
        try:
            if self.plugin_manager:
                ...
                # Fallback to basic plugin check
                return {
                    "status": "warning",
                    "loaded_plugins": 5,  # Default plugins
                    "enabled_plugins": 5,
                    "plugin_names": ["tasks", "client", "habit", "reminder", "calendar"],
                    "note": "Plugin manager not available"
                }
        except Exception as e:
            return {
                "status": "critical",
                "error": f"Plugin check failed: {str(e)}",
                "loaded_plugins": 0,
                "enabled_plugins": 0
            }
    ...
    async def execute(self, operation: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if operation == "get_system_health":
                ...
                return self._create_success_response(result, "System health retrieved successfully")
            elif operation == "get_quick_health":
                result = await self.get_quick_health()
                return self._create_success_response(result, "Quick health retrieved successfully")
            elif operation == "get_detailed_health":
                result = await self.get_detailed_health()
                return self._create_success_response(result, "Detailed health retrieved successfully")
            else:
                return self._handle_error(ValueError(f"Unknown operation: {operation}"))
        except Exception as e:
            return self._handle_error(e, f"Error in health service operation: {operation}") 