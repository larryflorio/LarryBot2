...
class TaskAttachmentService(BaseService):
    ...
    def __init__(self, task_attachment_repository: TaskAttachmentRepository, task_repository: TaskRepository):
        super().__init__()
        ...
        self.allowed_extensions = {'.pdf', '.doc', '.docx', '.txt', '.jpg', '.jpeg', '.png', '.gif', '.zip', '.rar'}
    ...
    async def attach_file(self, 
        ...
        try:
            ...
            if len(file_data) > self.max_file_size:
                return self._handle_error(ValueError(f"File too large. Maximum size: {self.max_file_size // (1024*1024)}MB"))
            
            # Validate filename first (before file type validation)
            ...
            if file_extension not in self.allowed_extensions:
                return self._handle_error(ValueError(f"File type not allowed: {file_extension}. Allowed: {', '.join(self.allowed_extensions)}"))
            
            # Add attachment
            attachment = self.task_attachment_repository.add_attachment(
                task_id=task_id,
                file_data=file_data,
    ...
    async def get_task_attachments(self, task_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            attachments = self.task_attachment_repository.get_attachments_by_task(task_id)
            stats = self.task_attachment_repository.get_attachment_stats(task_id)
    ...
    async def remove_attachment(self, attachment_id: int) -> Dict[str, Any]:
        ...
        try:
            attachment = self.task_attachment_repository.remove_attachment(attachment_id)
            if attachment:
                return self._create_success_response(
                    {
                        'id': attachment.id,
                ...
                return self._handle_error(ValueError(f"Attachment {attachment_id} not found"))
                
        except Exception as e:
            return self._handle_error(e, "Error removing attachment")
    
    async def update_attachment_description(self, attachment_id: int, description: str) -> Dict[str, Any]:
        """Update attachment description."""
        try:
            # Validate description
            if not description or len(description) > 1000:
                return self._handle_error(ValueError("Description must be between 1 and 1000 characters"))
            
            attachment = self.task_attachment_repository.update_attachment_description(attachment_id, description)
            if attachment:
                return self._create_success_response(
                    {
                        'id': attachment.id,
    ...
    async def get_attachment_stats(self, task_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = self.task_attachment_repository.get_attachment_stats(task_id)
            return self._create_success_response(stats, "Statistics retrieved successfully")
    ...
    async def get_attachment_by_id(self, attachment_id: int) -> Dict[str, Any]:
        ...
        try:
            attachment = self.task_attachment_repository.get_attachment_by_id(attachment_id)
            if attachment:
                return self._create_success_response(
                    {
                        'id': attachment.id,
